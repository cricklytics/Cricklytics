    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-100">Club not found</h1>
          <Link
            to="/Clubsmain"
      ...
      <header className="bg-gradient-to-r from-gray-800 to-gray-700 text-white py-4 px-4 shadow-md">
        <div className="container mx-auto flex items-center">
          ...
          <div>
            ...
            <p className="text-gray-300 text-sm">{club.category} Club</p>
          </div>
        </div>
      </header>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6">
            ...
            <div className="md:col-span-1">
              <div className="flex flex-col items-center mb-6">
                <img
                  src={club.logo}
                  alt={`${club.name} logo`}
                  className="w-48 h-48 object-contain rounded-full border-4 border-gray-700 mb-4"
                />
                <div className="text-center">
                  <h2 className="text-xl text-white font-bold">{club.name}</h2>
              ...
              <div className="space-y-4">
                <div className="p-4 bg-gray-700 rounded-lg">
                  <h3 className="font-bold text-white mb-2">Club Information</h3>
                  <div className="space-y-2">
                    <div className="flex items-center">
                      <FiUsers className="text-gray-300 mr-2" />
                      <span className="text-gray-100">{club.members} members</span>
                    </div>
                    <div className="flex items-center">
                      <FiCalendar className="text-gray-300 mr-2" />
                      <span className="text-gray-100">Established {club.established}</span>
                    </div>
                    {club.category === 'Premier' && (
                      <div className="flex items-center">
                        <FiStar className="text-yellow-400 mr-2" />
                        <span className="text-gray-100">Premier Club</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="p-4 bg-gray-700 rounded-lg">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center">
                      ...
                      <a href={`mailto:${club.contact.email}`} className="text-blue-400 hover:underline">
                        {club.contact.email}
                      </a>
                    </div>
                    <div className="flex items-center">
                      <FiGlobe className="text-gray-300 mr-2" />
                      <a href={`https://${club.contact.website}`} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">
                    ...
                    <div className="flex items-center">
                      ...
                      <span className="text-blue-400">{club.contact.phone}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="md:col-span-2 space-y-6">
              <div className="bg-gray-700 p-4 rounded-lg">
                ...
                <p className="text-gray-200">{club.description}</p>
              </div>

              <div className="bg-gray-700 p-4 rounded-lg">
                <h2 className="text-lg font-bold text-white mb-2">Upcoming Matches</h2>
                <div className="space-y-3">
                  {club.upcomingMatches.length > 0 ? (
                    club.upcomingMatches.map((match, index) => (
                      <div key={index} className="bg-gray-600 p-3 rounded-md shadow-sm">
              ...
              <div className="bg-gray-700 p-4 rounded-lg">
                <h2 className="text-lg font-bold text-white mb-2">Facilities</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {club.facilities.map((facility, index) => (
                    <div key={index} className="bg-gray-600 p-2 rounded-md shadow-sm text-sm text-white">
                      {facility}
                    </div>
                  ))}
                </div>
                  ...
                  <ul className="space-y-2">
                    {club.achievements.map((achievement, index) => (
                      <li key={index} className="flex items-start">
                        <FiStar className="text-yellow-400 mt-1 mr-2 flex-shrink-0" />
