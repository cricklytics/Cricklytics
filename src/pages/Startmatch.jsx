...
const Flowchart = ({ teams, groups, currentPhase, matches, groupResults, tournamentWinner, oddTeam, phaseHistory, currentGroupIndex }) => {
  ...
  const getExpectedTeamCount = (phase) => {
    if (phase === 'league') {
      ...
      return teams.length === 7 || teams.length === 4 ? 4 : getExpectedTeamCount('quarter') > 4 ? 4 : 0;
    } else if (phase === 'final') {
      return 2;
      ...
      <div className="bg-gray-800 p-6 rounded-xl mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {groupMatches.map(match => (
            <div key={match.id} className={`bg-gray-700 p-4 rounded-lg ${match.played ? 'bg-green-800' : ''}`}>
  ...
  const phases = (teams.length === 7 || teams.length === 4 ? ['league', 'quarter', 'semi', 'final', 'winner'] : ['league', 'super', 'quarter', 'semi', 'final', 'winner']).filter(phase => 
    phase !== 'semi' || getExpectedTeamCount(teams.length === 7 || teams.length === 4 ? 'quarter' : 'super') > 4
  );
  const levels = phases.map(phase => ({
    ...
    matchesInfo: phase === 'league' ? `Groups: ${groups.map((_, i) => `Group ${i + 1} (${getGroupStandings(i).length} teams)`).join(', ')}${oddTeam ? ', Odd Team advances to Quarter-Final' : ''}` :
                 ...
                 phase === 'semi' ? teams.length === 7 || teams.length === 4 ? 'Top 4 teams from Quarter-Final' : 'Top 4 teams from Super stage' :
                 phase === 'final' ? getExpectedTeamCount(teams.length === 7 || teams.length === 4 ? 'quarter' : 'super') > 4 ? 'Winners from Semi-Final' : 'Top 2 teams from Super stage' :
...
const Startmatch = ({ initialTeamA = '', initialTeamB = '', onMatchSetupComplete, origin }) => {
  ...
  const { teams, groups: initialGroups, matches: initialMatches, currentPhase: initialPhase, currentGroupIndex: initialGroupIndex, winner } = location.state || {};
  const scorers = ['John Doe', 'Jane Smith', 'Mike Johnson'];

  const [teamsState, setTeamsState] = useState(teams || []);
  const [groups, setGroups] = useState(initialGroups || []);
  const [matches, setMatches] = useState(initialMatches || []);
  const [currentPhase, setCurrentPhase] = useState(initialPhase || 'league');
  ...
  useEffect(() => {
    const fetchAllTeams = async () => {
      try {
        ...
        if (teamsState.length === 0 && location.state?.teams) {
          ...
          initializeTournament('roundRobin', initialTeams);
        }
      } catch (err) {
  ...
  useEffect(() => {
    if (winner && location.state?.origin === '/tournament-bracket') {
      handleMatchResultFromStartMatch(winner);
    }
  }, [winner]);
  ...
  const initializeTournament = (formatType, teamsToUse) => {
    ...
    if (formatType === 'roundRobin') {
      ...
      newMatches = newGroups.flatMap((group, index) => generateGroupMatches(group, index, 'league'));
      setGroups(newGroups);
  ...
  const getAvailableMatches = () => {
    ...
    return matches
      .filter(m => m.group === currentGroupIndex && m.phase === currentPhase && !m.played)
      .map(m => ({
        id: m.id,
        team1: m.team1,
        team2: m.team2,
  ...
  const handleMatchResultFromStartMatch = (winnerTeamName) => {
    if (winnerTeamName === 'Tie') return;

    const currentMatch = matches.find(m => m.group === currentGroupIndex && m.phase === currentPhase && !m.played);
    if (!currentMatch) return;

    const winnerId = teamsState.find(t => t.name === winnerTeamName)?.id;
    if (!winnerId) return;
    ...
    setMatches(prevMatches => {
        ...
        if (match.id === currentMatch.id) {
          ...
          setGroupResults(prev => {
            ...
            if (winnerId === match.team1.id) {
              ...
              team1Stats.losses += 1;
              team2Stats.points += 2;
              team2Stats.runsFor += 200;
              team2Stats.runsAgainst += 180;
              team1Stats.runsFor += 180;
              team1Stats.runsAgainst += 200;
            }
      ...
      if (currentGroupMatches.every(m => m.played)) {
        if (currentGroupIndex < groups.length - 1 && currentPhase === 'league') {
          ...
          if (currentPhase === 'league') {
            if (teamsState.length === 7) {
              const qualifiedTeams = groups.flatMap((group, groupIndex) => {
                const sorted = groups[groupIndex]
                  .map(team => ({
                    ...team,
                    ...groupResults[team.id]
                  }))
                  .sort((a, b) => (b.points || 0) - (a.points || 0) || (b.netRunRate || 0) - (a.netRunRate || 0) || (b.wins || 0) - (a.wins || 0));
            ...
            } else if (teamsState.length === 4) {
              const qualifiedTeams = groups[0]
                .map(team => ({
                  ...team,
                  ...groupResults[team.id]
                }))
              ...
              const teamsPerGroup = teamsState.length === 9 ? 2 : 3;
              const qualifiedTeams = groups.flatMap((group, groupIndex) => {
                const sorted = groups[groupIndex]
                  .map(team => ({
                    ...team,
          ...
          } else if (currentPhase === 'quarter' && teamsState.length === 7) {
            initializeSemiFinal();
  ...
  const initializeSemiFinal = () => {
    const qualifiedTeams = groups[0]
      .map(team => ({
        ...team,
        ...groupResults[teamsState.length === 7 ? 'quarter' : 'super']
      }))
      .sort((a, b) => (b.points || 0) - (a.points || 0) || (b.netRunRate || 0) - (a.netRunRate || 0) || (b.wins || 0) - (a.wins || 0))
      .slice(0, 4);
    ...
    setPhaseHistory(prev => [...prev, teamsState.length === 7 ? 'quarter' : 'super']);
    setCurrentGroupIndex(0);
    setSelectedMatch('');
    setSelectedTeamA('');
    setSelectedTeamB('');
  ...
  const initializeFinal = () => {
    ...
    if (phaseHistory.includes('semi')) {
      finalTeams = matches
        .filter(m => m.phase === 'semi' && m.winner)
        .map(m => teamsState.find(t => t.id === m.winner));
    } else {
      finalTeams = groups[0]
        .map(team => ({
          ...team,
          ...groupResults[currentPhase === 'league' ? 'league' : 'super']
        }))
        .sort((a, b) => (b.points || 0) - (a.points || 0) || (b.netRunRate || 0) - (a.netRunRate || 0) || (b.wins || 0) - (a.wins || 0))
        .slice(0, 2);
    }
    ...
    setPhaseHistory(prev => [...prev, phaseHistory.includes('semi') ? 'semi' : teamsState.length === 7 ? 'quarter' : 'super']);
    setCurrentGroupIndex(0);
    setSelectedMatch('');
    setSelectedTeamA('');
    setSelectedTeamB('');
              ...
              >
                ...
                <div className="space-y-4 w-full flex-1">
                  <div className="w-full">
                    <label className="block text-gray-700 mb-2 font-medium">Available Matches (Group {currentGroupIndex + 1})</label>
                    <select
                      id="battleselection"
