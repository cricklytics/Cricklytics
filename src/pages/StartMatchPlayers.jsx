...
function StartMatchPlayers() {
  ...
  return (
    <section className="min-h-screen w-full flex flex-col items-center py-[10rem] pb-[5rem]" style={{
        ...
        <>
          ...
          {!bowlerVisible && isChasing && (
            <div id="bowling" className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10 mt-10">
              {playerNames.map((name, index) => (
                <div key={index} onClick={() => handlePlayerSelect(index)} className={`cursor-pointer flex flex-col items-center text-white text-center ${selectedBatsmenIndices.includes(index) ? 'opacity-50' : ''}`}>
                  <div className="w-24 h-24 md:w-32 md:h-32 lg:w-40 lg:h-40 rounded-full border-[5px] border-[#white] overflow-hidden flex items-center justify-center">
                    <img src={playerImages[index]} alt="Player" className="w-full h-full object-cover rounded-full" />
                  </div>
          ...
          {!bowlerVisible && !isChasing && (
            <div id="batting" className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10 mt-10">
              {playerNames.map((name, index) => (
                <div key={index} onClick={() => handlePlayerSelect(index)} className={`cursor-pointer flex flex-col items-center text-white text-center ${selectedBatsmenIndices.includes(index) ? 'opacity-50' : ''}`}>
                  <div className="w-24 h-24 md:w-32 md:h-32 lg:w-40 lg:h-40  rounded-full border-[5px] border-[#F0167C] overflow-hidden flex items-center justify-center">
                    <img src={playerImages[index]} alt="Player" className="w-full h-full object-cover rounded-full" />
                  </div>
                  <span className="mt-2 font-bold text-lg">{name}</span>
                  <h2 className="text-sm font-light">{playerRoles[index]}</h2>
          ...
          {selectedBowler && (
            <div className="relative inline-block text-center text-white">
              <img src={selectedBowler.image} alt="Bowler" className="w-24 h-24 md:w-32 md:h-32 lg:w-40 lg:h-40  rounded-full mx-auto" />
          ...
          {bowlerVisible && (
            <>
              <div id={isChasing ? "bowling" : "bowling"} className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10 mt-10">
                {playerNames.map((name, index) => (
                  <div key={index} onClick={() => isChasing ? handleBowlerSelect(index) : handleBowlerSelect(index)} className={`cursor-pointer flex flex-col items-center text-white text-center ${selectedBowler?.index === index ? 'opacity-50' : ''}`}>
                    <div className={`w-24 h-24 md:w-32 md:h-32 lg:w-40 lg:h-40  rounded-full border-[5px] ${isChasing ? 'border-[#12BFA5]' : 'border-[#12BFA5]'} overflow-hidden flex items-center justify-center`}>
                      <img src={playerImages[index]} alt="Player" className="w-full h-full object-cover rounded-full" />
                    </div>
                    <span className="mt-2 font-bold text-lg">{name}</span>
                    <h2 className="text-sm font-light">{playerRoles[index]}</h2>
         ...
         <div id="start" className="relative flex flex-col w-full h-full items-center px-4 mt-5 md:mt-20 ">
                  ...
                  <div className="mt-10 flex  md:flex-row w-full md:w-1/2 justify-around gap-20 h-fit pt-5">
                    ...
                    <div className=" flex items-center justify-center mb-4 md:mb-0">
                      ...
                      <img src={isChasing ? flag1 : flag2} className="w-16 h-16 md:w-30 md:h-30" alt="Flag" />
                    </div>
                  </div>
                  ...
                  <div className="w-full flex flex-col md:justify-between md:flex-row md:w-[50%] justify-around mt-6 md:mt-10 md:pr-15">
                    ...
                    <div className='flex flex-col justify-center items-center w-full md:w-[20%] mb-6 md:mb-0'>
                      <div className="flex justify-center">
                        <button onClick={() => setShowPastOvers(!showPastOvers)} className="w-24 md:w-32 h-10 md:h-12 bg-[#4C0025] text-white font-bold text-sm md:text-lg rounded-lg border-2 border-white">
                              ...
                              <div key={index} className="bg-[#4C0025] p-2 md:p-3 rounded-lg">
                                ...
                                <div className="flex gap-1 md:gap-2">
                                  {over.map((ball, ballIndex) => (
                                    <span key={ballIndex} className={`w-4 h-4 md:w-5 md:h-5 lg:w-6 lg:h-6 flex items-center justify-center rounded-full text-xs md:text-sm ${ball === 'W' || ball === 'O' ? 'bg-red-600' : 'bg-[#FF62A1]'}`}>
                                      {ball}
                                    </span>
                                  ))}
                                </div>
                  ...
                  <div className="mt-4 md:mt-6 flex flex-wrap justify-center gap-2 md:gap-4">
                    {['Wide', 'No-ball', 'OUT', 'Leg By','lbw'].map((label) => (
                      <button key={label} onClick={() => handleScoreButtonClick(label, true)} className={`w-20 h-10 md:w-24 md:h-12 ${label === 'OUT' ? 'bg-red-600' : 'bg-[#4C0025]'} text-white font-bold text-sm md:text-lg rounded-lg border-2 border-white flex items-center justify-center`}>
                        {label}
                      </button>
                    ))}
                  </div>
                  ...
                  {showBatsmanDropdown && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                      <div className="bg-[#4C0025] p-4 md:p-6 rounded-lg max-w-md w-full mx-4">
                        <h3 className="text-white text-lg md:text-xl font-bold mb-4">Select Next Batsman</h3>
                        <div className="grid grid-cols-2 gap-2 md:gap-4">
                          {getAvailableBatsmen().map((player) => (
                            <div key={player.index} onClick={() => handleBatsmanSelect(player.index)} className="cursor-pointer flex flex-col items-center text-white text-center p-2 hover:bg-[#FF62A1] rounded-lg">
                              <img src={player.image} alt="Player" className="w-12 h-12 md:w-16 md:h-16 rounded-full" />
                        ...
                        <div className="grid grid-cols-2 gap-2 md:gap-4">
                          {playerNames.map((name, index) => (
                            index !== selectedBowler?.index && (
                              <div key={index} onClick={() => handleBowlerSelect(index)} className="cursor-pointer flex flex-col items-center text-white text-center p-2 hover:bg-[#FF62A1] rounded-lg">
                                <img src={playerImages[index]} alt="Player" className="w-12 h-12 md:w-16 md:h-16 rounded-full" />
